 ...
 */
class DropdownItem extends UI.UIBehavior {
    list;
    data;
}
class DropdownList {
  ...
  */
    ...
    addItem(data, index = -1) {
        let itemUI = this._cache.length > 0 ? this._cache.shift() : UI.UIManager.instance.create(this._itemCls);
        if (!itemUI.list) {
            itemUI.list = this;
        ...
        itemUI.uiObject.visibility = UI.SlateVisibility.SelfHitTestInvisible;
        ...
        this._root.cmdPanel.size = new Type.Vector2(270, this._items.length * (itemUI.uiObject.size.y + this.space));
        this._invalidateLayout();
    }
    /**
     * 删除一个选项
    ...
    _invalidateLayout() {
        if (this._isDropdown) {
            let offset = 0;
            this._root.cmdPanel.position = Type.Vector2.zero;
            this._root.dropList.visibility = UI.SlateVisibility.SelfHitTestInvisible;
            for (let i = 0; i < this._items.length; i++) {
                this._items[i].uiObject.position = new Type.Vector2(0, offset);
                offset += this._items[i].uiObject.size.y + this.space;
            }
        }
        else {
            this._root.dropList.visibility = UI.SlateVisibility.Collapsed;
        }
    }
}
...
function AddGMCommand(label, cmdClient, cmdServer, group, shortKey) {
    ...
    if (Util.SystemUtil.isServer()) {
        GMService.instance.getAuthority();
    }
    else {
        if (shortKey != null) {
...
class GMService {
    ...
    constructor() {
        if (SystemUtil.isServer()) {
            Events.addClientListener("GM_CALL", (player, gmIndex, param) => {
                GMConfig[gmIndex].serverCmd(player, param);
            });
            Events.addClientListener("GM.CheckAuthority", (player) => {
                const inter = setInterval(() => {
                    if (this.isGetGMUserList) {
                        clearInterval(inter);
                        Events.dispatchToClient(player, "GM.CheckAuthority", this.userList.includes(player.getUserId()));
                    }
                }, 500);
            });
            ...
            Events.addServerListener("GM.CheckAuthority", (isAuthority) => {
    ...
    getAuthority() {
        ...
        DataStorage.setTemporaryStorage(SystemUtil.isPIE);
        DataStorage.asyncGetCustomData("GM.UserList").then((userList) => {
            if (!userList) {
                DataStorage.asyncSetCustomData("GM.UserList", []);
            }
            else {
     ...
     * @param callback 回调
     */
    checkGMAuthority(callback) {
        Events.dispatchToServer("GM.CheckAuthority");
    ...
    cmd(data, param) {
        ...
        Events.dispatchLocal("GM_RUN", data.label);
        const index = GMConfig.indexOf(data);
        if (index >= 0 && data.serverCmd) {
            Events.dispatchToServer("GM_CALL", index, param);
        }
    }
}
...
(function (GM) {
     ...
     */
    function checkAuthority(callback) {
        GMService.instance.checkGMAuthority(callback);
    }
...
class GMBasePanel {
    ...
    constructor(_viewCls, _itemCls) {
        ...
        this._view = UI.UIManager.instance.create(_viewCls);
